...
//생성은 hullshader에서 들어온 정점 그대로 반환
...
//패치상수 함수 
PatchInfo PatchFunc(InputPatch<VS_IN, 3> _patch, int _patchIndx : SV_PrimitiveID)
{
    PatchInfo output = (PatchInfo) 0.f;
    
    //들어온 정점 16등분
    //삼각형 패치(3개의 정점을 잇는 사이 선을 16등분해서 정점들을 생성
...
//Hull __Shader
//정점을 생성 hullshader함수와 패치상수함수가 한쌍
struct HS_OUT
{
    float3 vPos : POSITION;
    float2 vUV : TEXCOORD;
...
[outputcontrolpoints(3)] //출력 제어점으로 정의된 3개의 출력 제어점이 있습니다
[maxtessfactor(64)]
             ...
             , OutputPatch<HS_OUT, 3> _patch, float3 _location : SV_DomainLocation)
{
    ...
    //정점의 위치는 호출된 패치의 정점 위치 * 위치 비율을 통해서 보간처리
