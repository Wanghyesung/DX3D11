...
VS_OUT VS_ParticleRender(VS_IN _in)
{
    ...
    //.vPos = mul(float4(_in.vPos, 1.f), g_matWorld);///////////
...
{    
    ...
    //for (int i = 0; i < 4; ++i)
    //{
    //    NewPos[i] = mul(float4(NewPos[i], 0.f), g_matWorld);
    //}
    ...
    {        
        if (ModuleData.VelocityScale)
        {
         ...
         //카메라가 바라보는 방향으로 파티클오브젝트를 회전
        
         //원점 카메라에서 바라보는 내 포지션
         //vParticleViewPos
        
         //카메라 앞부분 (0,0,1) 벡터
         //카메라에서 픽셀 벡터는 vParticleViewPos normalize
         //내적 = 각도 각도값만큼 회전행렬 연산
    ...
    //회전 모듈이 켜져있으면
    //if (ModuleData.Rotate)
    //{
    //    for (int i = 0; i < 4; ++i)
    //    {
    //        NewPos[i] = mul(float4(NewPos[i], 0.f), ModuleData.vRotateMatrix);
    //    }
    //}
