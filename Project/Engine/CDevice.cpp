...
int CDevice::CreateDepthStencilState()
{
    ...
    //LESS라면 가장 가까운 픽셀이 화면에 그려지게 되고 먼 픽셀은 폐기된다.
    // Less Equal
    ...
    // cullback로
    //볼륨 메쉬의 frontFace 보다 뒤에 있는 영역에 stencol 값 유지
    Desc.DepthEnable = true;
    ...
    Desc.FrontFace.StencilFunc = D3D11_COMPARISON_EQUAL; //스텐실이 1인 얘들만 호출
    Desc.FrontFace.StencilPassOp = D3D11_STENCIL_OP_KEEP;
    Desc.FrontFace.StencilFailOp = D3D11_STENCIL_OP_KEEP;
    Desc.FrontFace.StencilDepthFailOp = D3D11_STENCIL_OP_DECR; //깊이테스트 실패 -> 스텐실 값 감소

    //최종 타겟에는 stencol값이 1인 부분이 겹친 부분으로 나타남
    

    // 볼륨 메쉬내부 판정, 스텐실값 변경 (CULL_NONE) 2번으로 나누지 말고 한번에 해결하는 방법
    Desc.DepthEnable = true;
    Desc.DepthFunc = D3D11_COMPARISON_GREATER; //렌더링 되는 볼륨메쉬의 앞면이 더 멀어야 통과
    Desc.DepthWriteMask = D3D11_DEPTH_WRITE_MASK_ZERO; //볼륨메쉬는 깊이를 남기지 않음
    ...
    Desc.FrontFace.StencilDepthFailOp = D3D11_STENCIL_OP_INCR;   // 깊이 테스트 실패 -> 스텐실 값 증가

    //최종적으로 스텐실이 2인 얘들만 데칼영역으로 표시됨
...
int CDevice::CreateBlendState()
{
    ...
    //DEFFEREED DECAL BLEND
    // 
    //내 알파는 0곱해서 목적지 알파는1곱해서 기존 색으로
    Desc.AlphaToCoverageEnable = false;
    Desc.IndependentBlendEnable = true; //독립적으로
