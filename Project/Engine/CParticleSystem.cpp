	...
	, m_AccTime(0.f)
{
	...
	m_ModuleData.SpawnRate = 55;
	m_ModuleData.vSpawnColor = Vec3(0.0f, 0.f, 0.0f);
	m_ModuleData.vSpawnScaleMin = Vec3(25.f, 25.f, 1.f);
	m_ModuleData.vSpawnScaleMax = Vec3(30.f, 30.f, 1.f);
	...
	m_ModuleData.EndScale = 0.5f;

	m_ModuleData.ModuleCheck[(UINT)PARTICLE_MODULE::COLOR_CHANGE] = false;
	m_ModuleData.vStartColor = Vec3(1.f, 0.f, 0.f);
	m_ModuleData.vEndColor = Vec3(0.1f, 0.f, 0.f);
	...
	m_ModuleData.vMaxSpeed = 1000.f;


	m_ModuleData.ModuleCheck[(UINT)PARTICLE_MODULE::ROTATE] = true;
	m_ModuleData.vRotateMatrix = {};
...
{
	//local모드에 따라서 파티클의 좌표를 어떻게 할지를 결정하기 위해서
	...
	// Particle Render	
	Ptr<CTexture> pParticleTex = CResMgr::GetInst()->Load<CTexture>(L"Particle_0", L"texture\\particle\\fire.png");
